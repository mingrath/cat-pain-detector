...
- [ ] Deploy live demo publicly, likely Hugging Face Spaces or another GPU-backed endpoint, and keep Kaggle Notebook as a reproducible fallback. A temporary Gradio share was tested and then taken offline because live scoring was not close enough. Relaunch only after `docs/accuracy_recovery_plan.md` validation gates pass; stable HF Space remains blocked until a Hugging Face token/account is configured.
...
Current recovery execution status:

- [x] Disable public numeric score display by default in the Gradio report.
